...
The six example groups below are this article's authored selection, not an exhaustive or universally agreed scientific taxonomy. Their panels compare different prediction targets and uses; selecting a panel is not a benchmark comparison between the named systems. Try each group in turn and the panel swaps what it predicts; the JEPA group carries an explicit no-decoder marker.
