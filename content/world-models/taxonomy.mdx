...
Here the world model and the policy fuse into one backbone that emits future frames and action chunks together, so world modeling acts as an auxiliary objective that shapes the representations the policy reads. WorldVLA interleaves action and image tokens autoregressively and reports that the two objectives improve each other <Cite id="worldvla-2025" />; Cosmos Policy is the same idea on top of the Cosmos backbone. The prediction is real, but its job is to make the policy better, not to be inspected or rolled out interactively.
