...
Imagination has a catch, and it bounds everything below. Drag the horizon slider out and the deviation readout climbs faster than the horizon does, because each imagined step conditions on the model's own previous prediction. That is why DreamerV3 dreams fifteen steps and TD-MPC2 plans three.
...
<div className="my-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
  ...
  <Stat label="TD-MPC2 agent" value="317M" note="one agent, 80 tasks" />
  <Stat label="DayDreamer" value="1 h" note="to walk from scratch" />
  <Stat label="horizon" value="15" note="imagined steps, DreamerV3" />
</div>
...
The catch is the same one that limits every paradigm in this domain: error compounds. A one-step error does not stay a one-step error. It becomes part of the state the next prediction is read from, and deviations grow superlinearly down the rollout, which is why DreamerV3 trains its actor and critic on 15-step imagined rollouts and TD-MPC2 plans only 3 steps ahead <Cite id="dreamerv3-2023" /><Cite id="tdmpc2-2023" />.

The figure at the top of this module has both cases. Its Dreamer mode shows what the drift does to decoded frames. Its TD-MPC2 mode shows the decoder-free situation: there is no frame to degrade, and the quantity that matters is reward prediction error, the scalar the planner actually optimizes.
