...
4. **Action-free pretraining is not the robot-control stage.** V-JEPA 2-AC uses robot video with end-effector state signals and constructs action vectors from successive state differences. The excluded labels are rewards, task types, and success annotations <Cite id="vjepa2-2025" />.
