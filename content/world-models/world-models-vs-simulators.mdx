...
Many productive systems combine them. Physics supplies structure and broad counterfactual coverage. Learning supplies appearance, actuator residuals, contacts or task distributions that are expensive to specify.
...
The LLM column anchors the scale. An 8B dense decoder such as Llama 3 8B stores about 16 GB of weights in fp16 (8B parameters at two bytes each); at this scale, decoding is typically dominated by reading those weights from memory rather than by compute <Cite id="llama-3-2024" />. The Llama 3 paper gives the 8B model 32 layers and 8 key-value heads under grouped-query attention <Cite id="llama-3-herd-2024" />. In fp16 that works out to a KV cache of about 128 KB per token (32 layers x 8 key-value heads x 128-dimensional keys and values at two bytes each, from the paper's architecture table), so memory grows linearly with context length.

The latent column is where planning gets cheap enough to run on a robot. V-JEPA 2-AC pairs a frozen 1B-parameter ViT-g video encoder with a 300M-parameter block-causal predictor, so the whole planning stack is about 1.3B parameters, a few GB in fp16 <Cite id="vjepa2-2025" />. Its planner runs the cross-entropy method with 800 samples over 10 refinement iterations and produces an action in about 16 seconds, while a Cosmos video-generation planner took about 4 minutes per action with only 80 samples. The paper reports the two times, not a ratio; they work out to roughly 15x <Cite id="vjepa2-2025" />. Cheap per step does not mean flat in horizon. The predictor is block-causal: each patch feature attends to the patch features, actions and end-effector states of the current and earlier steps, so a longer rollout carries a longer context, though of 16 x 16 latent patch maps rather than pixels. Both planners in that comparison looked one step ahead <Cite id="vjepa2-2025" />.

The generative-video column is the expensive one. Cosmos Predict spans autoregressive models at 4B to 13B and diffusion models at 7B to 14B over continuous and discrete video tokenizers <Cite id="cosmos-predict-2025" />. NVIDIA's own measurements put Predict2's 14B Video2World variant at 56 GB of VRAM and roughly 85 seconds on a GB200 to over 30 minutes on a DGX Spark per clip at 480p and 16 fps, with 720p slower still <Cite id="cosmos-predict2-perf-2025" />; the Predict1 14B Video2World card lists over 80 GB of GPU memory for a five-second 1280x704 clip in its configuration that still offloads the prompt upsampler <Cite id="cosmos-1-diffusion-14b-card-2025" />. These are offline generators rather than 24 fps runtimes, and a policy that calls one per action step inherits that latency.

The explicit-3D column inverts the trade. 3D Gaussian Splatting stores the scene as rasterizable primitives, so a query is a raster pass: no weights, no tokens, and tens of milliseconds or better per frame on ordinary GPUs <Cite id="3dgs-2023" />. Memory moves to the asset itself: the paper reports 270 to 734 MB per scene for its tested scenes, which use 1 to 5 million Gaussians. The limitation is semantic rather than computational: a splat answers what a view looks like, not what an action does, which is why the real-to-sim-to-real pattern below attaches explicit physics.
