...
The LLM column anchors the scale. An 8B dense decoder such as Llama 3 8B keeps about 16 GB of weights in fp16 and emits a token in tens of milliseconds on a consumer card; at that size, decode is bound by reading weights rather than compute <Cite id="llama-3-2024" />. The Llama 3 paper gives the 8B model 32 layers and 8 key-value heads under grouped-query attention <Cite id="llama-3-herd-2024" />. In fp16 that works out to a KV cache of about 128 KB per token, so memory grows linearly with context length.
