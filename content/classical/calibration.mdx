...
<SelfCheck
  prompt="A grasp misses by almost the same small offset in one direction across many objects, and the miss grows as targets move farther from the wrist camera. What should you inspect first?"
  options={[
    {
      value: 'hand-eye',
...
Hand-eye calibration solves the rigid relationship between camera motion and robot motion. For a wrist camera, the unknown is usually the transform from gripper to camera; for a fixed camera, it is base to camera. Shiu and Ahmad framed wrist-mounted sensor calibration as solving homogeneous transform equations of the form $AX = XB$ <Cite id="shiu-ahmad-1989" />; Tsai and Lenz gave the classic efficient solution <Cite id="tsai-lenz-1989" />.
...
For learned policies, perform alignment before forming observation-action pairs. A fixed temporal shift changes the apparent control law and can make a policy look temporally unstable.
