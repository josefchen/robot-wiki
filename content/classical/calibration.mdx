...
Hand-eye calibration solves the rigid relationship between camera motion and robot motion. For a wrist camera, the unknown is usually the transform from gripper to camera; for a fixed camera, it is base to camera. Tsai and Lenz formulated the classic $AX = XB$ problem from paired robot and camera motions <Cite id="tsai-lenz-1989" />.
