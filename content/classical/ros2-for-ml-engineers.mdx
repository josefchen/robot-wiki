...
At the policy boundary, transform observations and actions deliberately. If training used base-frame end-effector deltas, deployment must not send tool-frame deltas with the same tensor layout. The geometry behind these transformations is developed in *Modern Robotics* <Cite id="modern-robotics-2017" />.
