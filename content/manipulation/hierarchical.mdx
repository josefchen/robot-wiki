...
The shift is visible as a timing pattern. Four 2026 systems are drawn below as lanes on one two-second timeline. Drag the playhead across it: the motor lanes tick constantly while the subtask lanes barely move, and switching systems with the four buttons shows the same split instantiated four different ways.
...
The timeline at the top of this module draws four of these systems, including Helix 02's 200 Hz and 1 kHz lanes and GO-2's asynchronous planner/follower split. Rates tagged "schematic" are not stated in the primary source; the lane exists because the architectural split is disclosed, but the exact frequency is not.
