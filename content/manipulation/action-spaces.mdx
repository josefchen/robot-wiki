...
An action chunk predicts several future steps per inference. ACT showed that this temporal abstraction can matter more than the choice of probabilistic head <Cite id="act-aloha-2023" />. Diffusion Policy also predicts a horizon and executes only the first portion before replanning <Cite id="diffusion-policy-2023" />.
