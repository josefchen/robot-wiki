...
One failed espresso episode is walked below. Drag the episode time slider through it and watch the value trace: rising segments get tagged high advantage, falling segments low, and the dashed arc is the credit assignment, a failed insertion blamed on the grasp twenty seconds earlier. Learning from that failure without backpropagating through a flow-matching action head is the problem this module solves.
...
The other two views of the figure at the top of this module show what Recap does with those tags: at training time every transition is kept with its binary label, and at execution time the policy is conditioned on "high."
