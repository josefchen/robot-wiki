...
const sourceIds = ['realsense-tuning-2026', 'azure-kinect-depth-docs-2026'];
...
for (const viewport of [{ width: 375, height: 812 }, { width: 1440, height: 900 }]) {
  test.describe(`perception depth readers at ${viewport.width}px`, () => {
    test.use({ viewport });

    test('depth corrections, preserved Stat, real calculator and drawer remain readable', async ({ page }, info) => {
    ...
    test('References page bylines and unchanged depth glossary retain source identities and history', async ({ page }, info) => {
      await open(page, info);
      const history = [];
      ...
      // Neither corrected source is cited by a glossary definition; this depth term is a no-change guard.
