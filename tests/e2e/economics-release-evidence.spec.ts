...
import { DIRECTORY, ROUTE, BROWSER, artifact, dependencies, defaults, ranges, oracle, save } from '../../audit/evidence/economics-release-20260923/support';
import { setSlider } from './slider';

const producing = process.env.ECONOMICS_RELEASE_BROWSER === '1';
...
test('industrial52 actual default robot-cost endpoints and reset', async ({ page, staticBase }) => {
  ...
  if (producing) save('browser-run.json', {
    command: 'NODE_DISABLE_COMPILE_CACHE=1 ECONOMICS_RELEASE_BROWSER=1 node_modules/.bin/playwright test --config=playwright.economics-release.config.ts tests/e2e/economics-release-evidence.spec.ts --workers=1 --retries=0 --reporter=line --output=audit/evidence/economics-release-20260923/playwright-final',
