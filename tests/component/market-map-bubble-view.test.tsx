...
describe('BubbleView hover, focus, and roving keyboard access', () => {
  it(
    'reveals name and plotted value on hover and clears on leave',
    async () => {
  ...
  it(
    'reveals the same label on keyboard focus (hover/focus parity)',
    () => {
      render(<BubbleView companies={COMPANIES} />);
  ...
  it(
    'marks the chart a single tab stop with a roving tabindex',
    () => {
  ...
  it(
    'moves between marks spatially with the arrow keys',
    async () => {
      const user = userEvent.setup();
      const points = bubblePointsById();
      render(<BubbleView companies={COMPANIES} />);
  ...
  it(
    'keeps Enter and Space selection working (VAL-MKT-023 regression)',
    () => {
      render(<BubbleView companies={COMPANIES} />);
      const figure = mark('figure-ai');
...
describe('BubbleView deep-link highlight parity', () => {
  it(
    'treats the hashed company as selected: accent mark plus detail panel',
    () => {
  ...
  it(
    'keeps a manual selection after the highlight changes',
    () => {
  ...
  it(
    'keeps exactly one tab stop when the hash names an unplotted company',
    () => {
      ...
      const tabbable = COMPANIES.filter((company) => company.id !== 'covariant')
        .map((company) =>
          document.querySelector(`circle[data-company-id="${company.id}"]`),
        )
        .filter(
          (el) => el instanceof Element && (el as SVGElement).tabIndex === 0,
        );
      expect(tabbable).toHaveLength(1);
      expect(
        document.querySelector('circle[data-company-id="covariant"]'),
      ).toBeNull();
    },
    TIMEOUT,
  );
  ...
  it(
    'exposes the selected mark id on the chart root (plotted deep link)',
    () => {
      render(<BubbleView companies={COMPANIES} highlightedId="figure-ai" />);
  ...
  it(
    'leaves selection unset when the hash names an unplotted company',
    () => {
...
describe('BubbleView focus ring', () => {
  it(
    'renders an explicit focus ring element around the focused mark',
    () => {
      render(<BubbleView companies={COMPANIES} />);
      const figure = mark('figure-ai');
  ...
  it(
    'keeps the aria-label contract of the marks unchanged',
    () => {
      render(<BubbleView companies={COMPANIES} />);
      const figure = mark('figure-ai');
  ...
  it(
    'keeps the last-focused mark as the tab stop after blur (roving tabindex)',
    () => {
      render(<BubbleView companies={COMPANIES} />);
  ...
  it(
    'renders the focus ring outside the clip group so extreme marks are never clipped',
    () => {
      render(<BubbleView companies={COMPANIES} />);
      const svg = document.querySelector('svg[role="group"]');
      expect(svg).not.toBeNull();
