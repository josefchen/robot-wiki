...
import { DEFAULT_MARKET_MAP_FILTERS, filterCompanies } from '@/lib/market-map';
...
describe('MarketMap', () => {
  it(
    'renders all 112 companies grouped by the six segments (VAL-MKT-001, VAL-MKT-002)',
    () => {
      ...
      expect(
        screen.getByRole('heading', { name: /Humanoids/ }),
  ...
  it(
    'exposes all seven filter dimensions and the three views (VAL-MKT-003, VAL-MKT-004)',
    async () => {
      ...
      expect(
        screen.getByRole('button', { name: 'Bubble' }),
      ).toBeInTheDocument();
      expect(
  ...
  it(
    'applies a single filter and composes a second one (VAL-MKT-005, VAL-MKT-006)',
    async () => {
      const user = userEvent.setup();
      render(<MarketMap companies={COMPANIES} />);
  ...
  it(
    'narrows sub-segments when a segment is active (VAL-MKT-025)',
    async () => {
      const user = userEvent.setup();
      render(<MarketMap companies={COMPANIES} />);
  ...
  it(
    'shows the complete field set and expands inline (VAL-MKT-009, VAL-MKT-010)',
    async () => {
      const user = userEvent.setup();
      render(<MarketMap companies={COMPANIES} />);
  ...
  it(
    'renders unknown funding as not disclosed, never zero (VAL-MKT-011)',
    () => {
      render(<MarketMap companies={COMPANIES} />);
      const covariant = card('Covariant');
      expect(
        within(covariant).getAllByText('not disclosed').length,
      ...
      expect(
        genesis.querySelector('[data-field="valuation"]')?.textContent,
      ).toBe('not disclosed');
    },
    TIMEOUT,
  );
  ...
  it(
    'switches to bubble and timeline views (VAL-MKT-003, VAL-MKT-015, VAL-MKT-023)',
    async () => {
      const user = userEvent.setup();
      render(<MarketMap companies={COMPANIES} />);
  ...
  it(
    'shows an empty state and restores the full set on clear (VAL-MKT-017, VAL-MKT-018)',
    async () => {
      const user = userEvent.setup();
      render(<MarketMap companies={COMPANIES} />);
      ...
      await user.selectOptions(
        screen.getByLabelText('Stage / status'),
        'shut-down',
      );
      expect(
        screen.getByText('No companies match these filters.'),
      ).toBeInTheDocument();
  ...
  it(
    'hydrates filters from the URL and ignores invalid params (VAL-MKT-007, VAL-MKT-024)',
    async () => {
  ...
  it(
    'drops unknown subSegment/country/approach params but keeps valid siblings',
    async () => {
      window.history.replaceState(
        null,
        '',
  ...
  it(
    'relaxes filters that exclude the company named in the hash',
    async () => {
      window.history.replaceState(
        null,
        '',
  ...
  it(
    'clears every filter when the hashed company fails all of them',
    async () => {
      window.history.replaceState(
        null,
        '',
  ...
  it(
    'does not relax again after the hash has been honored',
    async () => {
      const user = userEvent.setup();
  ...
  it(
    'leaves filters alone when the hash names an unknown company',
    async () => {
      window.history.replaceState(
        null,
        '',
  ...
  it(
    'announces the result count in an aria-live region (VAL-MKT-026)',
    () => {
      render(<MarketMap companies={COMPANIES} />);
