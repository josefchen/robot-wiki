...
describe('source-backed visual-servo origin pair', () => {
  it('binds both exact originals without completing held calibration or absence records', () => {
    ...
    // The later paper-body recovery completed 9/17 with separate reviewed
    // Tsai–Lenz plans; an IEEE abstract alone would not prove either whole row.
    for (const ordinal of [9, 17]) {
      const plan = plans.find(p => p.rowOrdinal === ordinal && p.articleSlug === 'perception')!;
      expect(plan.id).toBe(`perception-${ordinal}-tsai-${ordinal === 9 ? 'solve' : 'protocol'}-20260916g`);
      expect(plan.evidence).toHaveLength(1);
      expect(plan.adjudications).toHaveLength(plan.parts.length);
      expect(perception.claimRecords[ordinal - 1].evidenceFailures).toEqual([]);
    }
  });
