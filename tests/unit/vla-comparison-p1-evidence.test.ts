  ...
  it('preserves unselected rows/plans and archives the exact old selected objects', () => {
    ...
    for (const section of old) section.claimRecords.forEach((record, i) => {
      if (section.slug === 'generalist-policies' && i === 18) {
        const now = current.find(s => s.slug === section.slug)!.claimRecords[i];
        expect(now.verdict).toBe('UNRESOLVED (bounded local-text correction only; external-passage requirement remains unmet)');
        const history = now.note.match(/^Historical four-cell record retained: (.+) Correction rationale:/);
        expect(history).not.toBeNull();
        expect(originalClaimDigest(JSON.parse(history![1]))).toBe(originalClaimDigest(record));
        expect(now.evidenceFailures.length).toBeGreaterThan(0);
