...
describe('environment-trap script wiring', () => {
  ...
  it('prunes sync-conflict duplicates from the .next type dirs before every build', () => {
    ...
    // .next/dev/types is not, and tsconfig includes both). The Tektur
    // binary gate runs after content validation and before the build.
    expect(pkg.scripts.prebuild).toBe(
      'node scripts/prune-next-type-duplicates.ts && node scripts/prune-generated-og-shadows.ts && npm run validate:content && npm run check:tektur-fonts',
    );
  });
