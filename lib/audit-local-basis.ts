...
import { WM_PARADIGMS } from './world-model-taxonomy.ts';
...
const perceptionInputs = z.object({
  ...
  poseMm: finite.min(0).max(20), workingDistanceM: finite.min(.15).max(1.5),
  target: z.enum(['opaque', 'specular', 'transparent']),
}).strict();
const selectedPerceptionHeadings = [
...
const recipeSchema = z.union([
  ...
  z.discriminatedUnion('id', [
    z.object({ id: z.literal('generalist-selection'), mode: z.literal('derive'),
      inputs: z.object({ ids: z.array(text).length(13) }).strict() }).strict(),
    z.object({ id: z.literal('taxonomy-selection'), mode: z.literal('derive'),
      inputs: z.object({ ids: z.array(text).length(6) }).strict() }).strict(),
    z.object({ id: z.literal('rrt'), mode: z.literal('derive'),
      inputs: z.object({ iteration: z.number().int().min(0).max(900) }).strict() }).strict(),
    z.object({ id: z.literal('perception'), mode: z.literal('derive'), inputs: perceptionInputs }).strict(),
    z.object({ id: z.literal('perception-ray'), mode: z.literal('derive'),
      inputs: z.object({ angleDeg: finite.min(0).max(3), distanceM: z.union([z.literal(.1), z.literal(1)]) }).strict() }).strict(),
    z.object({ id: z.literal('perception-stages'), mode: z.literal('derive'),
      inputs: z.object({ headings: z.array(text).length(6) }).strict() }).strict(),
...
type Target = { component: string | null; recipe: RecipeId; mounts: readonly number[] };
export const LOCAL_BASIS_REQUIRED_TARGETS: Readonly<Record<string, Target>> = Object.freeze({
  'audit/manipulation.md:generalist-policies:19': { component: null, recipe: 'generalist-selection', mounts: [] },
  'audit/world-models.md:taxonomy:4': { component: null, recipe: 'taxonomy-selection', mounts: [] },
  'audit/classical.md:motion-planning:15': { component: 'RrtExplorer', recipe: 'rrt', mounts: [1] },
  'audit/classical.md:perception:1': { component: 'PerceptionErrorBudget', recipe: 'perception', mounts: [1] },
  'audit/classical.md:perception:2': { component: 'PerceptionErrorBudget', recipe: 'perception', mounts: [1] },
  'audit/classical.md:perception:3': { component: 'PerceptionErrorBudget', recipe: 'perception', mounts: [1] },
  'audit/classical.md:perception:7': { component: 'PerceptionErrorBudget', recipe: 'perception', mounts: [1] },
  'audit/classical.md:perception:19': { component: null, recipe: 'perception-ray', mounts: [] },
  'audit/classical.md:perception:59': { component: null, recipe: 'perception-stages', mounts: [] },
...
export function recomputeLocalDerivation(input: unknown): z.infer<typeof outputSchema> {
  ...
  if (recipe.mode === 'parameters') {
    switch (recipe.id) {
      case 'generalist-selection': values = { ids: GENERALIST_RELEASES.map(r => r.id) };
        units = 'author-selected IDs, not source facts'; break;
      case 'taxonomy-selection': values = { ids: WM_PARADIGMS.map(r => r.id) };
        units = 'author-selected IDs, not scientific paradigms'; break;
      case 'rrt': values = { scene: rrt.RRT_SCENE, defaults: rrt.RRT_DEFAULTS,
        iterations: { opening: 0, step: 1, playback: 3, scrub: 100 },
        cadence: { normal: rrt.playbackCadence(false), reduced: rrt.playbackCadence(true) } };
        units = 'abstract world units; accepted extensions:count; probability:1'; break;
      case 'perception': values = { defaults: perception.DEFAULT_PARAMS, sliders: perception.SLIDER_SPECS,
        nominalRangeM: perception.NOMINAL_RANGE_M, referencePct: perception.PUBLISHED_DEPTH_SPEC_PCT,
        multipliers: perception.TARGET_CLASSES.map(t => ({ id: t.id, multiple: t.specMultiple })),
        geometry: { openingMm: 90, partMm: 60 }, bandsMm: [perception.CLEARANCE_MM, 2 * perception.CLEARANCE_MM],
        cases: {
          opening: perception.DEFAULT_PARAMS,
          near: { ...perception.DEFAULT_PARAMS, workingDistanceM: .15 },
          far: { ...perception.DEFAULT_PARAMS, workingDistanceM: 1.5 },
          zeroNear: { ...perception.DEFAULT_PARAMS, handEyeDeg: 0, workingDistanceM: .15 },
          zeroFar: { ...perception.DEFAULT_PARAMS, handEyeDeg: 0, workingDistanceM: 1.5 },
          specular: { ...perception.DEFAULT_PARAMS, target: 'specular' },
          transparent: { ...perception.DEFAULT_PARAMS, target: 'transparent' },
        } }; units = 'deg,m,mm,% as named; authored geometry, not empirical tolerance'; break;
      case 'perception-ray': values = { angles: { one: 1, zero: 0 }, distances: { near: .1, far: 1 },
        geometry: 'Right triangle: tan(theta)=lateral offset/axial plane separation; not a rigid-transform norm.' };
        units = 'angle:deg; distance:m'; break;
      case 'perception-stages': values = { headings: selectedPerceptionHeadings }; units = 'authored heading selection'; break;
    ...
    switch (recipe.id) {
      case 'generalist-selection':
      case 'taxonomy-selection': {
        const ids = recipe.id === 'generalist-selection' ? GENERALIST_RELEASES.map(r => r.id) : WM_PARADIGMS.map(r => r.id);
        requireThat(same(recipe.inputs.ids, ids), 'selected IDs differ from the exact authored list');
        distinct(ids, 'selected ID');
        values = { ids, count: ids.length };
        units = 'author-selected records:count; not scientific/source certification';
        precision = 'exact integer'; formula = 'cardinality of the exact duplicate-free authored ID list'; break;
      }
      case 'rrt': {
        const tree = rrt.buildRrt(rrt.RRT_SCENE);
        const iteration = Math.min(recipe.inputs.iteration, tree.nodes.length - 1);
        const goalReached = rrt.pathIfReached(tree, iteration).length > 0;
        const status = goalReached ? `goal reached at iteration ${tree.goalNodeId}` : iteration === 0 ? 'tree not started' : 'exploring free space';
        values = { totalExtensions: tree.nodes.length - 1, visibleNodes: rrt.nodesUpTo(tree, iteration).length,
          visibleEdges: rrt.edgesUpTo(tree, iteration).length, goalReached, goalNodeId: tree.goalNodeId,
          pathLength: tree.pathLength, treeDigest: digest(tree),
          display: [`${iteration} / ${tree.nodes.length - 1}`, String(iteration + 1), status,
            goalReached ? `${rrt.formatLength(tree.pathLength)} units` : 'n/a'] };
        units = 'abstract world units; accepted extensions:count';
        precision = 'path length:toFixed(1); counts:integer'; break;
      }
      case 'perception': {
        const budget = perception.composeBudget(recipe.inputs);
        const depth = budget.contributions.find(c => c.key === 'depth')!.mm;
        const band = { within: 'within model band', marginal: 'marginal', jam: 'above model band' }[budget.verdict];
        values = { ...budget, slackMm: (90 - 60) / 2,
          bandsMm: [perception.CLEARANCE_MM, 2 * perception.CLEARANCE_MM],
          display: [`${budget.totalMm.toFixed(2)} mm`, `${depth.toFixed(2)} mm at ${budget.effectiveDepthPct.toFixed(1)}%`, band] };
        units = 'contributions,total,slack,bands:mm; depth:%; shares:dimensionless squared-input fractions';
        precision = 'readout:mm toFixed(2), percent toFixed(1); not a statistical variance';
        formula += ';sqrt((d*1000*tan(theta*pi/180))^2+(.5*1000*max(depthPct,2*multiple)/100)^2+poseMm^2)'; break;
      }
      case 'perception-ray': {
        const mm = perception.handEyeErrorMm(recipe.inputs.angleDeg, recipe.inputs.distanceM);
        values = { mm, rounded: mm.toFixed(recipe.inputs.distanceM === .1 ? 1 : 0) };
        units = 'offset:mm; input angle:deg, axial distance:m';
        precision = '0.1m example:nearest 0.1mm; 1m example:nearest 1mm';
        formula = 'tan(theta)=opposite/adjacent; offsetMm=distanceM*1000*tan(angleDeg*pi/180)'; break;
      }
      case 'perception-stages': {
        requireThat(same(recipe.inputs.headings, selectedPerceptionHeadings), 'wrong authored heading selection');
        const article = readFileSync(join(import.meta.dirname, '../content/classical/perception.mdx'), 'utf8').split(/\r?\n/);
        const occurrences = recipe.inputs.headings.map(h => article.filter(line => line === h).length);
        requireThat(occurrences.every(n => n === 1), 'selected heading absent or duplicated');
        values = { headings: recipe.inputs.headings, count: new Set(recipe.inputs.headings).size, occurrences };
        units = 'selected unique headings:count'; precision = 'exact integer';
        formula = 'cardinality of the declared six-heading selection, not all headings'; break;
      }
...
const INPUT_UNITS: Record<RecipeId, Record<string, string>> = {
  'generalist-selection': { ids: 'authored-ID-selection' }, 'taxonomy-selection': { ids: 'authored-ID-selection' },
...
function checkProof(plan: LocalPlan, proof: LocalProof, context: LocalBasisContext, read: (a: LocalArtifact) => Buffer): void {
  ...
  requireThat(target.component === null || source, 'source missing from enumeration');
  for (const path of [...LOCAL_RECIPE_DEPENDENCIES[target.recipe], ...(source ? [source.sourcePath] : []), articlePath,
