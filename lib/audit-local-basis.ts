...
function readRetainedDependency(root: string, ref: LocalArtifact): Buffer {
  ...
  requireThat(historical[ref.path] === ref.sha256, `artifact bytes/hash: ${ref.path}`);
  const review = JSON.parse(readBoundedLocalFile(root, 'audit/evidence/industrial-closure-20260923/dependency-review.json').toString());
