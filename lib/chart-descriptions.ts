 ...
 *
 * Population note: the source sweep in the gate script counts MOUNTS in
 * component files (43 across 38 files at last measure); the RENDERED DOM
 * carries more described chart roots (63) because several components map
 * one root per item in a loop. Both numbers are right about different
 * things. This registry is indexed by the SOURCE-mount population; any
 * browser-side assertion must state which population it is counting.
 */
